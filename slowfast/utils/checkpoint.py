...
def load_checkpoint(
    ...
    finetune=False
    epoch_reset=False, # this one is recent addition
    clear_name_pattern=(), # this one too
):
    """
...
def load_train_checkpoint(cfg, model, optimizer, finetune=False):
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        checkpoint_epoch = load_checkpoint(
            ...
            convert_from_caffe2=cfg.TRAIN.CHECKPOINT_TYPE == "caffe2",
            finetune=finetune
            epoch_reset=cfg.TRAIN.CHECKPOINT_EPOCH_RESET, # recent addition-when cloned my repo
            clear_name_pattern=cfg.TRAIN.CHECKPOINT_CLEAR_NAME_PATTERN, #recent
        )
        start_epoch = checkpoint_epoch + 1
    else:
        start_epoch = 0
